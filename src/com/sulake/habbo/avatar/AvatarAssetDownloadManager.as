package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_402:AssetLoaderStruct;
      ...
      private const const_1959:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         if(_loc6_ == null)
         {
            this.var_402 = this._assets.loadAssetFromFile("figuremap",_loc5_,"text/xml");
         ...
         this.var_869 = new Timer(this.const_1959,1);
         this.var_869.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_402)
         {
            this.var_402.addEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_402.addEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_402)
         {
            this.var_402.removeEventListener(AssetLoaderEvent.const_28,this.onConfigurationComplete);
            this.var_402.removeEventListener(AssetLoaderEvent.const_44,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         else
         {
            ...
            if(this.var_1317.indexOf("?") > 0)
            {
               _loc2_ = this.var_1317 + "&retry=" + this.var_870;
            }
            else
            {
               _loc2_ = this.var_1317 + "?retry=" + this.var_870;
            ...
            this.var_402 = this._assets.loadAssetFromFile("figuremap",_loc3_,"text/xml");
            this.addMapLoaderEventListeners();
         }
      }
      ...
      private function loadFigureMapData(param1:XML) : void
      {
         ...
         this.var_1623 = true;
         for each(_loc2_ in this.var_1074)
         {
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2150,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1075[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1075[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1623)
         {
            this.var_1074.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_511[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_511[_loc3_] = _loc6_;
            }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         var _loc10_:* = null;
         var _loc2_:* = [];
         for(_loc3_ in this.var_868)
         {
            _loc7_ = true;
            _loc4_ = this.var_868[_loc3_];
            ...
            if(_loc7_)
            {
               ...
               delete this.var_511[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_868[_loc3_];
         }
         while(_loc6_ < this.var_692.length)
         {
            _loc5_ = this.var_692[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_692.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_869.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_691.length > 0 && this.var_692.length < this.const_1478)
         {
            _loc1_ = this.var_691[0];
            _loc1_.startDownloading();
            this.var_692.push(this.var_691.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_691.indexOf(param1) == -1 && this.var_692.indexOf(param1) == -1)
         {
            this.var_691.push(param1);
            this.processPending();
         }
      }
