package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_338 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_568 && this.var_1327)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_90,this.var_880,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_34,parseInt(FigureData.const_775));
         ...
         if(this.var_338)
         {
            if(this.var_338.bitmap)
            {
               this.var_338.bitmap.dispose();
            }
            this.var_338.bitmap = new BitmapData(this.var_338.width,this.var_338.height,true,0);
            _loc7_ = (this.var_338.width - _loc1_.width) / 2;
            _loc8_ = (this.var_338.height - _loc1_.height) / 2;
            this.var_338.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            case "set_button":
               ...
               this.var_880 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1641,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_568)
               {
                  this._controller.loadAvatarInEditor(this.var_568,this.var_880,this._controller.clubMemberLevel);
               }
         }
      }
