package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private var var_1102:IContainerButtonWindow;
      
      private var var_1103:IContainerButtonWindow;
      ...
      private function prepareWindow() : void
      {
         ...
         this.var_1663 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1103 = this.var_1663.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1103)
         {
            this.var_1103.procedure = this.onButtonClick;
         }
         this.var_1102 = this.var_1663.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1102)
         {
            this.var_1102.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_246 >= this.var_81.length - 1)
         {
            this.var_1102.disable();
         }
         else
         {
            this.var_1102.enable();
         }
         if(this.var_246 <= 0)
         {
            this.var_1103.disable();
         }
         else
         {
            this.var_1103.enable();
         }
      }
      ...
      {
         ++this.var_246;
         this.var_246 = Math.min(this.var_246,this.var_81.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_246;
         this.var_246 = Math.max(this.var_246,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         while(this.var_89.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            ...
            this.var_81.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_81 || this.var_81.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_81[this.var_246][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_81[this.var_246][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_81[this.var_246][1]).value;
            this.setPreviewFromAsset(_loc4_);
         }
      }
      
      private function method_5(param1:IBitmapWrapperWindow, param2:BitmapData) : void
      {
         if(param2 == null)
         {
            return;
      ...
      private function setPreviewFromAsset(param1:String) : void
      {
         ...
         if(_loc3_)
         {
            this.method_5(_loc3_,_loc2_.content as BitmapData);
         }
      }
