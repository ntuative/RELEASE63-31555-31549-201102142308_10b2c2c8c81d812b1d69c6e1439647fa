package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2409 = new RoomThumbnailRenderer(param1);
         this.var_999 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_174 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_174.visible = true;
         Util.hideChildren(this.var_174);
         this.refreshRoomName(this.var_174,this.var_114);
         if(this.var_114.ownerName != "" && this.var_114.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_174,!!this.var_114.event ? this.getEventCtg(this.var_114.categoryId) : this.getRoomCtg(this.var_114.categoryId));
         this.refreshRoomDesc(this.var_174,this.var_114);
         this.refreshExtraCont();
         this.refreshInfo(this.var_174,"trading_allowed",this.var_114.allowTrading);
         this.refreshInfo(this.var_174,"doormode_doorbell",this.var_114.doorMode == 1);
         this.refreshInfo(this.var_174,"doormode_password",this.var_114.doorMode == 2);
         Util.moveChildrenToColumn(this.var_174,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_174.height = Util.getLowestPoint(this.var_174);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_174,this.var_114.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_174.findChildByName("roomowner"));
         if(this.var_114.ownerName != "" && this.var_114.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_174.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_999.refreshTags(_loc1_,this.var_114.tags);
         ...
         this.var_2409.refreshThumbnail(_loc1_,this.var_114.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_216,Tabs.const_233,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
