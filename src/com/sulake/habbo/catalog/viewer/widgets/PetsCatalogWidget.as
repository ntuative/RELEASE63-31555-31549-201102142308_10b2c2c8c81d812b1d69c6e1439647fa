package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_394 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         if(this.var_394 >= 8)
         {
            return false;
         }
         ...
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_394,this);
               if(_loc9_)
               {
                  this.var_205 = _loc9_.sellableBreeds;
                  this.var_358 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_205.length)
         {
            _loc10_ = this.var_205[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_394,_loc10_.id),this.getRaceLocalizationKey(this.var_394,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_394,_loc5_);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_358)
         {
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_57)
         {
            ...
            if(_loc3_ >= this.var_205.length)
            {
               return;
            }
            this.var_1808 = _loc3_;
            this.updateImage();
         }
      }
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         if(this.var_502 < 0 || this.var_502 > this.var_358.length)
         {
            this.var_502 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_502 >= 0 && this.var_502 < this.var_358.length)
         {
            _loc7_ = this.var_358[this.var_502] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_205[this.var_1808] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_394,_loc3_.id,_loc2_,AvatarScaleType.const_53,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_299,AvatarAction.const_767);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_502 >= this.var_358.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_358[this.var_502] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_205[this.var_1808] as IBreed;
