package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class NewPetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IGetImageListener
   {
      ...
      private static const const_1233:int = 18;
      ...
      private var var_205:Array = null;
      ...
      override public function init() : Boolean
      {
         ...
         this.var_394 = this.getPetTypeIndexFromProduct(_loc2_.localizationId);
         if(this.var_394 < 8)
         {
            return false;
         }
         this.updateAvailableBreeds(this.var_394);
         if(this.var_205 != null && this.var_205.length > 0)
         {
            this.var_329 = this.var_205[0];
         }
         else
         {
            this.var_329 = -1;
         }
         this._offers.add(this.var_394,_loc2_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_785,this.onApproveNameResult);
      ...
      private function initializeBreedSelection() : void
      {
         ...
         if(this.var_205 != null)
         {
            _loc1_ = [];
            _loc2_ = 0;
            while(_loc2_ < const_1233 && _loc2_ < this.var_205.length)
            {
               _loc3_ = (page.viewer.catalog as HabboCatalog).roomEngine.getPetColor(this.var_394,this.var_205[_loc2_]);
               if(_loc3_ != null)
               {
      ...
      private function selectedBreed(param1:int) : void
      {
         if(this.var_205 != null && this.var_205.length > 0)
         {
            if(param1 < 0 || param1 > this.var_205.length)
            {
               param1 = 0;
            }
            this.var_329 = this.var_205[param1];
            this.updateImage();
         }
      }
      ...
      private function getPetLocalization() : String
      {
         ...
         if(page != null && false)
         {
            ...
            {
               return _loc1_.localization.getKey(this.getRaceLocalizationKey(this.var_394,this.var_329),this.getRaceLocalizationKey(this.var_394,this.var_329));
            }
         }
         return this.getRaceLocalizationKey(this.var_394,this.var_329);
      }
      ...
      {
         if(param1.petType == this.var_394)
         {
            this.var_205 = param1.sellableBreeds;
            this.initializeBreedSelection();
      ...
      private function updateImage() : void
      {
         ...
         if(_loc1_ == null || this.var_329 < 0)
         {
            return;
         }
         var _loc2_:int = this.var_394;
         var _loc3_:int = this.var_329;
      ...
      private function getPurchaseParameters() : String
      {
         ...
         var p:String = this._name + String.fromCharCode(10) + String(this.var_329) + String.fromCharCode(10) + this.addZeroPadding(color.toString(16).toUpperCase(),6);
         return p;
      }
      ...
      private function updateAvailableBreeds(param1:int) : void
      {
         if(this.var_205 == null)
         {
            this.var_205 = (page.viewer.catalog as HabboCatalog).getSellablePetBreeds(param1);
         }
      }
