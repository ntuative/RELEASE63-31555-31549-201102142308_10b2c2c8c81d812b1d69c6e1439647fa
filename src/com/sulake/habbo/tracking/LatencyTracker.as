package com.sulake.habbo.tracking
{
   ...
   public class LatencyTracker
   {
      ...
      public function init() : void
      {
         ...
         this.var_1818 = int(this.var_164.getKey("latencytest.interval"));
         this.var_1821 = int(this.var_164.getKey("latencytest.report.index"));
         this.var_2562 = int(this.var_164.getKey("latencytest.report.delta"));
      ...
      public function update(param1:uint, param2:int) : void
      {
         ...
         if(param2 - this.var_1819 > this.var_1818)
         {
            this.testLatency();
         }
      }
      ...
      private function testLatency() : void
      {
         ...
         this.var_610.add(this.var_1418,this.var_1819);
         this._connection.send(new LatencyPingRequestMessageComposer(this.var_1418));
         ++this.var_1418;
      }
      ...
      private function onPingResponse(param1:IMessageEvent) : void
      {
         ...
         var _loc10_:int = 0;
         var _loc11_:* = null;
         if(this.var_610 == null || this.var_223 == null)
         {
            return;
         }
         var _loc2_:LatencyPingResponseMessageParser = (param1 as LatencyPingResponseMessageEvent).getParser();
         var _loc3_:int = this.var_610.getValue(_loc2_.requestId);
         this.var_610.remove(_loc2_.requestId);
         var _loc4_:int = getTimer() - _loc3_;
         this.var_223.push(_loc4_);
         if(this.var_223.length == this.var_1821 && this.var_1821 > 0)
         {
            _loc5_ = 0;
            _loc6_ = 0;
            _loc7_ = 0;
            ...
            if(Math.abs(_loc9_ - this.var_1820) > this.var_2562 || this.var_1820 == 0)
            {
               this.var_1820 = _loc9_;
