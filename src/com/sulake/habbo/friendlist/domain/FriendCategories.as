package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_360);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_415.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_415.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_415.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_415.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_415.view.setNewMessageArrived();
                  this.var_415.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_360).received = true;
         this.findCategory(FriendCategory.const_1068).received = true;
