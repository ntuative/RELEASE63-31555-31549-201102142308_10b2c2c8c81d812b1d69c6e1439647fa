package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      private var var_155:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_129:SSOTicket;
      ...
      {
         this.var_155.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_155 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_155.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_122,HabboWindowStyle.const_40,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_155.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1689,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_155.habboConfiguration)
            {
               if(this.var_155.habboConfiguration.getKey("use.sso","HabboNavigatorCom_roc_create_room_xml") == "true")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_155.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_155.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1689,"/");
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_155.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_129.removeEventListener(SSOTicket.const_353,this.onParsedTicketSuccess);
            this.var_129.removeEventListener(SSOTicket.const_278,this.onParsedTicketFailure);
            this.var_129 = null;
         }
         var _loc2_:String = "";
         if(this.var_155.habboConfiguration != null)
         {
            _loc2_ = this.var_155.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_155.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_129 = new SSOTicket(this.var_155.assets,_loc3_,_loc2_,param1);
         this.var_129.addEventListener(SSOTicket.const_353,this.onParsedTicketSuccess);
         this.var_129.addEventListener(SSOTicket.const_278,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_129.ticket);
         Logger.log("Has Facebook: " + this.var_129.isFacebookUser);
         if(this.var_129.isFacebookUser)
         {
            this.var_155.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_155.shockwaveClientUrl = this.var_129.shockwaveClientUrl;
         this.var_155.ssoTicket = this.var_129.ticket;
         this.var_129.removeEventListener(SSOTicket.const_353,this.onParsedTicketSuccess);
         this.var_129.removeEventListener(SSOTicket.const_278,this.onParsedTicketFailure);
         this.var_129 = null;
         dispatchEvent(new Event(const_475));
